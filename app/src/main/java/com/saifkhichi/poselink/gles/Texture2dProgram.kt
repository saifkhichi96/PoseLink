...
class Texture2dProgram(programType: ProgramType) {
    ...
    val kernel: FloatArray = FloatArray(KERNEL_SIZE)
    private lateinit var mTexOffset: FloatArray
    var colorAdjust: Float = 0f
        private set
    ...
    init {
        ...
        when (programType) {
            ...
            ProgramType.TEXTURE_EXT_FILT_VIEW -> {
                mTextureTarget = GLES11Ext.GL_TEXTURE_EXTERNAL_OES
    ...
    fun setKernel(values: FloatArray, colorAdj: Float) {
        ...
        System.arraycopy(values, 0, this.kernel, 0, KERNEL_SIZE)
        this.colorAdjust = colorAdj
    }
    ...
    fun draw(
        ...
        if (muKernelLoc >= 0) {
            GLES20.glUniform1fv(muKernelLoc, KERNEL_SIZE, this.kernel, 0)
            GLES20.glUniform2fv(muTexOffsetLoc, KERNEL_SIZE, mTexOffset, 0)
            GLES20.glUniform1f(muColorAdjustLoc, this.colorAdjust)
        }
    ...
    companion object {
        ...
        const val KERNEL_SIZE: Int = 9
        private val FRAGMENT_SHADER_EXT_FILT_VIEW =
