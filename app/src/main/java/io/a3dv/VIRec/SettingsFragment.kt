...
//                getPreferenceManager().findPreference("prefFocusDistance");
        ...
        try {
            ...
            for (i in 0..<cameraSize) {
                ...
                try {
                    if (characteristics.get<Int?>(CameraCharacteristics.LENS_FACING) ==
                        CameraMetadata.LENS_FACING_BACK
                    ) {
                        entries[i] = cameraId + " - Lens Facing Back"
                    } else if (characteristics.get<Int?>(CameraCharacteristics.LENS_FACING) ==
            ...
            val streamConfigurationMap = characteristics.get<StreamConfigurationMap?>(
                CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP
            )
            val sizes =
                streamConfigurationMap!!.getOutputSizes<MediaRecorder?>(MediaRecorder::class.java)
            ...
            val isoRange = characteristics.get<Range<Int?>?>(
            ...
            val exposureTimeRangeNs = characteristics.get<Range<Long?>?>(
                CameraCharacteristics.SENSOR_INFO_EXPOSURE_TIME_RANGE
            )
            if (exposureTimeRangeNs != null) {
                val exposureTimeRangeMs = Range<Float?>(
...
//            }
    ...
    private fun checkIso(newValue: Any): Boolean {
        if (newValue.toString() != "" && newValue.toString().matches("\\d*".toRegex())) {
            ...
            Toast.makeText(
                activity,
                newValue.toString() + " is not a valid number!", Toast.LENGTH_SHORT
            ).show()
            return false
        }
    }
    ...
    private fun switchPrefCameraValues(main: String?, SecondaryKey: String) {
        val list: ListPreference? =
            checkNotNull(preferenceManager.findPreference<ListPreference?>(SecondaryKey))
        val entryValues = list!!.entryValues
        val index = Arrays.asList<CharSequence?>(*entryValues).indexOf(main)
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences, key: String?) {
        if (key == "prefCamera") {
            try {
                ...
                val cameraRez: ListPreference? =
                    checkNotNull(preferenceManager.findPreference<ListPreference?>("prefSizeRaw"))
