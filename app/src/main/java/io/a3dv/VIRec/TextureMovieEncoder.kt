...
class TextureMovieEncoder : Runnable {
    ...
    private class EncoderHandler(encoder: TextureMovieEncoder?) : Handler() {
        ...
        init {
            mWeakEncoder = WeakReference<TextureMovieEncoder?>(encoder)
        }
    ...
    private fun prepareEncoder(
        ...
        if (program.programType == ProgramType.TEXTURE_EXT_FILT_VIEW) {
            val newProgram = Texture2dProgram(ProgramType.TEXTURE_EXT_FILT)
